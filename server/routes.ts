...
export async function registerRoutes(
  ...
  app.patch("/api/trips/:id", async (req, res) => {
    ...
    if (isNaN(tripId)) return res.sendStatus(400);
    try {
      const { name, photos, destination, startingLocation, startDate, endDate, expectations, activities, waveType, rideStyle, locationPreference, vibe, extraActivities, broadcastEnabled } = req.body;
      ...
      const trip = await storage.updateTrip(tripId, userId, { name, photos, destination, startingLocation, startDate, endDate, expectations, activities, waveType, rideStyle, locationPreference, vibe, extraActivities, broadcastEnabled });
      res.json(trip);
    } catch (err: any) {
